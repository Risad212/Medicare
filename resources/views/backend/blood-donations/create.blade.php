...
<div class="mc-head">
    <div>
        ...
        <h1 class="mc-title">Record <em>donation</em></h1>
        <p class="mc-sub">A new unit enters the pool: donor, group, quantity, and expiry.</p>
    </div>
</div>
...
<form action="{{ route('admin.blood-donations.store') }}" method="POST">
    ...
    <div class="mc-grid">
        <aside class="mc-side">
            <div class="mc-avbig">BD</div>
            <div class="k">New record</div>
            <h2>New donation</h2>
            <p>Starts at <b>Collected</b>; make it available after testing.</p>
            <ul class="mc-steps">
        ...
        <div>
                    ...
                    <div class="mc-f full">
                        ...
                        <select name="donor_id" class="form-select">
                            ...
                            @foreach($donors as $donor)
                                <option value="{{ $donor->id }}" {{ old('donor_id') == $donor->id ? 'selected' : '' }}>{{ $donor->name }} — {{ $donor->bloodGroup->name }}</option>
                            @endforeach
                        </select>
            ...
            <section class="mc-sec">
                ...
                <div class="mc-sec-bd">
                    <div class="mc-f">
                        ...
                        <input type="date" name="donation_date" class="form-control" value="{{ old('donation_date', now()->format('Y-m-d')) }}">
                    ...
                    <div class="mc-f">
                        ...
                        <input type="text" name="bag_number" class="form-control" value="{{ old('bag_number') }}" placeholder="e.g. B-00042">
                        @error('bag_number') <small class="text-red-t">{{ $message }}</small> @enderror
                    </div>
                    <div class="mc-f">
                        <label>Expiry date <i class="req">*</i></label>
                        <input type="date" name="expiry_date" class="form-control" value="{{ old('expiry_date', now()->addMonths(3)->format('Y-m-d')) }}">
                        @error('expiry_date') <small class="text-red-t">{{ $message }}</small> @enderror
                    </div>
                    <div class="mc-f full">
                        <label>Collection location</label>
                        <input type="text" name="collection_location" class="form-control" value="{{ old('collection_location') }}">
                    </div>
                    <div class="mc-f full">
                        <label>Notes</label>
            ...
            <div class="mc-formacts">
                <button type="submit" class="mc-btn"><i class="bi bi-save"></i> Save donation</button>
